...
export default function ImageGallery({ images }: any) {
  ...
  // Normalisation : accepte soit [{url:"..."}, ...] soit ["...", ...]
  const normalizedImages = (images ?? []).map((img: any) =>
    typeof img === 'string' ? img : img?.url,
  )

  const [currentImage, setCurrentImage] = useState(normalizedImages[0] ?? '')

  // S'assure qu'on met à jour si "images" change
  useEffect(() => {
    if (normalizedImages.length > 0) {
      setCurrentImage(normalizedImages[0])
    }
  }, [images])
      ...
      <div className="grid grid-cols-12 gap-4 h-auto md:h-[600px]">
        ...
        <div className="col-span-12 md:col-span-4 flex flex-row md:flex-col gap-4 overflow-x-auto md:overflow-y-auto scrollbar-hide pb-2 md:pb-0">
          ...
          {normalizedImages.map((image: any, index: number) => (
            <div
              key={index}
              className="w-32 h-32 md:w-full md:h-64 shrink-0 overflow-hidden rounded-xl"
