...
//   })
    ...
    // console.log("voici les projets recup", projects);
    ...
    <section className={`py-16 sm:py-24 bg-card ${isFullWidth === 'No' ? 'custom_container' : ''}`}>
      <div className="max-w-6xl mx-auto px-4 md:px-8 lg:px-14">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          {/* {response.docs.map((project, index) => ( */}
          {projects.map((project: any, index: any) => (
            <div key={index} className="group overflow-hidden rounded-lg">
              ...
              <div className="p-4 bg-background">
                <h3 className="font-bold text-lg">{project.title}</h3>
                <p className="text-sm text-text-muted">{project.projectType?.titre}</p>
              </div>
            </div>
          ))}
        </div>
