export default function TeamSection({
    ...
    return (
        <section className={`bg-card py-16 sm:py-24 ${isFullWidth === 'No' ? 'custom_container' : ''}`}>
            <div className="max-w-6xl mx-auto px-4 md:px-8 lg:px-14">
                ...
                <div className="mt-12 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:mt-16 lg:grid-cols-3 lg:gap-10">
                    {team_composition.map((member: any, index: number) => (
                        <div key={index} className="flex flex-col items-center text-center">
                            <img
                                className="h-40 w-40 rounded-full object-cover"
                                src={member.image?.url || member.image?.src || member.image || ''}
